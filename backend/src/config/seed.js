...
async function seed() {
  ...
  const adminAcct = generateAccountNumber();

  db.query(
    `INSERT OR IGNORE INTO users
       (id,email,password_hash,first_name,last_name,role,is_active,is_verified,account_number)
     VALUES (?,?,?,?,?,'admin',1,1,?)`,
    [adminId, 'admin@zentropay.local', adminPwd, 'Admin', 'User', adminAcct]
  );
  db.query(
  ...
  // Backfill account_number for admin if already existed without one
  db.query(
    `UPDATE users SET account_number=? WHERE email='admin@zentropay.local' AND account_number IS NULL`,
    [adminAcct]
  );
  ...
  for (const u of users) {
    ...
    db.query(
      `INSERT OR IGNORE INTO users
         (id,email,password_hash,first_name,last_name,role,is_active,is_verified,account_number)
       VALUES (?,?,?,?,?,'user',1,1,?)`,
      [uid, u.email, userPwd, u.fn, u.ln, acct]
    );
    // Backfill if user already existed without account_number
    db.query(
      `UPDATE users SET account_number=? WHERE email=? AND account_number IS NULL`,
      [acct, u.email]
    );
    db.query(
      `INSERT OR IGNORE INTO wallets (id,user_id,balance,currency)
  ...
  const descriptions = [
    'Product purchase', 'Service payment', 'Subscription renewal',
    'Invoice payment', 'Refund', 'Wallet top-up',
  ];
  ...
  for (let i = 0; i < 40; i++) {
    ...
    db.query(
      `INSERT OR IGNORE INTO transactions
         (id,user_id,amount,currency,status,description,payment_method,
